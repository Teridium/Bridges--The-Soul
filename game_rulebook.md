...
2.  **Фаза Нейтралов (в конце раунда):**
...
*   **Мастер (на все руки)**
    ...
    *   **Способности:**
        ...
        *   Может разрушать здания за 1 ОД, возвращая 50% стоимости (округляется вверх)
...
*   **Постройки (стоимость):**
    ...
    *   **Катапульта (2 монеты + 1 Пружина):** Можно зарядить за 1 ОД, после чего позволяет игрокам на этом острове совершать два бесплатных действия: **"Запуск Себя"**, чтобы переместиться на любой остров, и **"Запуск Предмета"**, чтобы отправить любой расходуемый предмет на любой остров, на любую цель, этот предмет активируется на эту цель. Эти действия тратят заряд катапульты. *Инструкция гласит: "1. Залезть в люльку. 2. Помолиться. 3. ??? 4. Профит"*
*   **Правила размещения:**
...
|:--- |:--- |:--- |:--- |:--- |:--- |
| **Крабулон** | 20*N МЗД | Когда в дальности атаки `Лазеров` появляются игроки хоть раз, он начинает стрелять по Базе `Лазерами`, если не может атаковать игрока. Использует и ближний и дальний бой в свой ход | **Ближний бой:** Если на его острове 2 или более игроков, использует `Размах клешней`. Иначе, использует `Сжать клешни`, также использует последнее, как контратаку вблизи<br>**Дальний бой:** Использует `Лазеры` по всем игрокам на соседнем острове, также использует их как дальнюю контратаку | `Размах клешней`: 2 УРН всем на острове<br>`Сжать клешни`: 4 УРН<br>`Лазеры`: 2 УРН, 1 ДАЛ, без ответа | **Цель игры** |
| **Големко** | 12*N МЗД | Становится мирным, если Карате-Бомж мертв (но снова атакует, если его атаковать) | Если был атакован дальнобойной атакой: атакует атаковавшего в ответ `Броском камня`<br>Если на его острове 2 или более игроков: использует `Удар по земле`, иначе: использует `Пинок`, также использует последнее, как контратаку вблизи | `Удар по земле`: 2 УРН всем на острове<br>`Пинок`: 3 УРН<br>`Бросок камня`: 1 УРН, 1 ДАЛ, без ответа | Латный доспех, Гигантский Щит Щитовика, 4 монеты |
| **Колдун Запеканка** | 10*N МЗД | Атакует только в ответ. Начинает атаковать самостоятельно, если Темный стрелок мертв | Если атакован издалека: отвечает `Лучом пекла`<br>Если на его острове 2 или более игроков: использует `Огненный шар` (не чаще 1 раза в 2 раунда), иначе: использует `Луч пекла`. Использует последнее, как контратаку | `Луч пекла`: 3 УРН, 1 ДАЛ, без ответа<br>`Огненный шар`: 1 УРН всем, без ответа | Посох Стихий, Зелье Лечения х4, 4 монеты |
| **Карате-бомж** | 6*N МЗД | Если на его острове лежат монеты, тратит ход на их сбор и не атакует | Если атакован издалека: отвечает `Карате-бутылкой`<br>В ближнем бою и в ближней контратаке использует `Карате-удар` | `Карате-удар`: 2 УРН, без ответа<br>`Карате-бутылка`: 1 УРН, 1 ДАЛ, без ответа | Раздвижная рука, 4 монеты, Бомба |
| **Темный стрелок Глаз-Алмаз** | 5*N МЗД | Не может атаковать, если на его острове есть хотя бы один игрок | В свой ход, если может, атакует `Выстрелом в глаз`, также отвечает им на дальние атаки<br>Если его атакуют вблизи: отвечает `Ударом` | `Удар`: 1 УРН<br>`Выстрел в глаз`: 2 УРН, 1 ДАЛ, без ответа | Арбалет, Пружина х2, 2 монеты |

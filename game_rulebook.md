...
5. **Стартовый набор:** 
    ...
    *   Количество всех предметов и монет умножается на количество игроков в команде
        ...
        *   3: `Посох Лесов`, `1 монета`, `Зелье лечения`
...
*   **Мастер (на все руки)**
    ...
    *   **Способности:**
        *   Возведение построек не требует ОД
        ...
        *   За 3 монеты может строить ускоритель бобхода, чтобы дать себе или другому игроку на этом же острове 1 ОД
...
*   **Дроп:**
    *   1: `Латный доспех`, `Зелье лечения`, 5 монет
    *   2: `Гигантский Щит Щитовика`, `Шарик-со-Щитом` x1, 3 монеты
    *   3: `Шарик-со-Щитом` x3, 3 монеты
    *   4: `Латный доспех`, `Гигантский Щит Щитовика`, 4 монеты
*   **Особое:**
...
*   **Умения:**
    ...
    *   `Чаёк`: Восстанавливает 1 ЗДР

### Колдун Запеканка
*   **Характеристики:** 10*N МЗД
*   **Дроп:**
    *   1: `Посох Лесов`, `Зелье Лечения` x2, 6 монет
    *   2: `Посох Стихий`, 2 монеты
    *   3: `Зелье Лечения` x6, 1 монета
    *   4: `Посох Стихий`, `Зелье Лечения` x4, 4 монеты
*   **Особое:**
    *   Атакует только в ответ, зато может отвечать на атаки "без ответа". Начинает атаковать самостоятельно, если Темный стрелок мертв
*   **Поведение с приоритетом:**
...
*   **Умения:**
    ...
    *   `Запеканки, кексы, вафли`: Востанавливает до 2 ЗДР

### Карате-бомж
*   **Характеристики:** 6*N МЗД
*   **Дроп:**
    *   1: `Двуручник`, `Зелье лечения` x2, 3 монеты
    *   2: `Раздвижная рука`, `Двуручник`, 2 монеты
    *   3: `Бомба` x2, 2 монеты
    *   4: `Раздвижная рука`, `Бомба`, 2 монеты
*   **Поведение с приоритетом:**
...
*   **Дроп:**
    *   1: `Рекурсивный лук`, `Пружина` x2, 4 монеты
    *   2: `Арбалет`, 4 монеты
    *   3: `Пружина` x4, 3 монета
    *   4: `Арбалет`, `Пружина` x2, 2 монеты
...
| **Обостренный панцирь** | `+1 МЗД`. Тот, кто атаковал носителя, получит 1 УРН даже если носитель умрет (не активируется, если урон по носителю заблокирован). Носитель может перемещаться только 1 раз за свою фазу | *Острее вашего языка!* |

### 5.5 Расходуемые предметы
| Название | Эффект и Описание |
|:--- |:--- |
| **Пружина** | Можно потратить, чтобы "перепрыгнуть" на любой другой остров<br>*ООО "Робинзон Грустно"* |
| **Бомба** | Ставится на остров за 1 ОД и наносит 5 УРН всем, кто стоит на нем на следующий ход<br>*Черный шарик с могучей силой внутри. Не открывать и не подносить к огню* |
